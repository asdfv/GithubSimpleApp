...
class UserDetailsDependenciesModule {
    ...
    UserDetailsDependenciesModule() {
        GetUserUseCase getUserUseCase = new GetUserUseCase(new IOThread(), new UIThread(), new UserDataRepository(new RetrofitUserDatasource(), new UserMapper()));
        UserDetailsItemMapper mapper = new UserDetailsItemMapper();
